...
class F1Score(Metric):
    def __init__(self, num_labels, threshold=0.5, average='samples', label_names=None):
        self.num_labels = num_labels
        self.threshold = threshold
        self.f1_score = MultilabelF1Score(num_labels=num_labels, average=average, threshold=threshold).to(device)
        self._f1 = 0.0
        self._count = 0
...
class SeqClassificationModel(Model):
    ...
    def __init__(self, vocab: Vocabulary,
       ...
       # define loss
        if self.sci_sum:
            ...
            # best pos_weight Clarin-Emo
            pos_weights = torch.tensor([1.0530414340287981, 1.1706958510290755, 1.228909465020576, 1.3116764275256223, 2.079802669762043,
                         2.409885675857431, 3.1796805678793256, 3.6529051987767582, 4.322677925211098, 8.676755447941888, 13.998046875])
            # pos_weight stockbrief
            # pos_weights = torch.tensor([1.2136204889406286, 1.5130624092888243, 3.475, 3.330670926517572, 4.695945945945946, 6.168639053254438,
            #                         8.34, 15.795454545454545])
            self.loss = torch.nn.BCEWithLogitsLoss(reduction='none', pos_weight=pos_weights)
            ...
            self.label_f1_metrics_macro = F1Score(num_labels=self.vocab.get_vocab_size(namespace='labels'), average="macro")
            self.label_f1_metrics_micro = F1Score(num_labels=self.vocab.get_vocab_size(namespace='labels'), average="micro")
            self.label_f1_metric = F1Score(num_labels=self.vocab.get_vocab_size(namespace='labels'), average=None, label_names=label_names)
    ...
    def forward(self,  # type: ignore
      ...
      if self.use_sep:
          ...
          if labels is not None:
              if self.labels_are_scores:
                  ...
                  final_k = []
                  for i in labels:
                      labels_mask = [row_idx for row_idx, row in enumerate(torch.split(i, 1, dim=0)) if not torch.all(row == 0)]
                      i = i[labels_mask, :]
                      final_k.append(i)
                  
              labels = torch.cat(final_k, dim=0)
      ...
      if labels is not None:
          ...
          flattened_gold = labels.float().view(-1, self.num_labels) #11 for clarin emo, 8 for stock brief
          ...
          if not self.with_crf:
              ...
              flattened_probs = torch.sigmoid(flattened_logits)
          else:
          ...
          N,C = labels.float().view(-1, self.num_labels).shape

          if not self.labels_are_scores:
              evaluation_mask = (flattened_gold != -1)
              flattened_probs = (flattened_probs>0.5).float()
              if flattened_probs.shape[0] == 1:
                  accuracy = (flattened_probs == labels.float().view(-1, self.num_labels)).sum() / (N*C)
